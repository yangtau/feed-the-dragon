...
class FieldControlled(Action):
    '''A state that a role is controlled by the force field generated by tools'''
    ...
    def to_pos(self):
        vx, vy = self.__speed
        return int(self.__x+vx), int(self.__y+vy)

    def update(self, a):
        vx, vy = self.__speed
        self.__x += vx
        self.__y += vy
        ax, ay = a  # self.__map.get_a(self.position)
        print(ax, ay)
        vx += ax
        vy += ay
        m = self.__map
        x, y = int(self.__x), int(self.__y)
        if vx > 0 and m.is_block((x + 48, y)) or \
                vx < 0 and m.is_block((x-48, y)):
            vx = -0.8*vx
        if vy > 0 and m.is_block((x, y+64+12)) or \
                vy < 0 and m.is_block((x, y+12)):
            vy = -0.5*vy
        self.__speed = vx, vy
        # image_size = (48, 64)
...
class Sprite(pygame.sprite.Sprite):
    ...
    def __init__(self, position: (int, int), config: str, default_action: Action = Idle(), m=None):
    ...
    def update(self):
        a = (0.0, 0.0)
        if self.__m:
            a = self.__m.get_a(self.position)
        if a != (0.0, 0.0):
            if not isinstance(self.__action, FieldControlled):
                self.__action = FieldControlled(self.__m)
                x, y = self.position
                x = (x//64)*64
                y = (y//64)*64
                self.__action.position = x, y
            self.__action.update(a)
